...
class PHPMSWordParser { 
	...
	public function setTemplateFile($origindocx) {
		...
		if ($origindocx) $this->templatefile = $origindocx;
	...
	public function setOutputFile($destinationdocx) {
		...
		if ($destinationdocx) $this->outputfile = $destinationdocx;
		if (!$this->outputfile) {
	...
	public function createFromTemplate($origindocx,$destinationdocx) {
		...
		if (!$this->setTemplateFile($origindocx)) return false;
		if (!$this->setOutputFile($destinationdocx)) return false;
	...
	private function __start() {
		...
		$this->__settempvars();
		if (!$this->__extractTemplate()) return false;
		...
		if (!$this->multi) $this->__cleartemp();
		$this->__resetvar();
	...
	private function __verbose($str,$only_when_debug=false) {
		...
		if ($this->verbose) {
			if ($only_when_debug && !$this->debug) return;
